...
export function AppSidebar() {
  ...
  // Lister plan → listing-only surface (vehicles, leads, marketplace, analytics)
  const mainMenuItems = useMemo(() => {
    ...
    return isLister ? items.filter((i) => ["/vehicles", "/leads"].includes(i.url)) : items;
  }, [newLeadsCount, isLister]);
  ...
  const managementItems = useMemo(
    () =>
      isLister
        ? managementMenuItems.filter((i) => i.url === "/analytics/public-page")
        : managementMenuItems,
    [isLister]
  );
    ...
    <Sidebar collapsible="icon" className="will-change-transform border-r border-border/30 overflow-x-hidden">
      <SidebarContent className="bg-sidebar flex flex-col h-full overflow-x-hidden">
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden scrollbar-invisible py-2">
          ...
          <MemoizedMenuGroup label="Management" items={managementItems} isCollapsed={isCollapsed} />
