...
import { reverseGeocodeDistrict, canonicalDistrict } from "@/lib/location";
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          // Resolve to a DISTRICT so it lines up with dealer-address filtering
          const { district } = await reverseGeocodeDistrict(latitude, longitude);

          if (district) {
            setDetectedCity(district);

            // Prefer a district we actually have listings in
            const match = availableCities.find(
              (c) => canonicalDistrict(c).toLowerCase() === district.toLowerCase()
            );

            if (match) {
              onCityChange(match);
              onLocationDetected?.(latitude, longitude, match);
              return;
            }
          }
