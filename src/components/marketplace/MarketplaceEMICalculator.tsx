...
export default function MarketplaceEMICalculator({ 
  ...
  // The slider range grows with the vehicle price so premium models
  // (₹50 lakh and above) are not clamped to the default ceiling.
  const sliderMin = Math.min(100000, Math.max(10000, Math.floor(price / 2)));
  const sliderMax = Math.max(5000000, Math.ceil((price * 1.5) / 100000) * 100000);
  const sliderStep = sliderMax > 10000000 ? 100000 : 10000;
  const shortInr = (n: number) =>
    n >= 10000000 ? `₹${(n / 10000000).toFixed(n % 10000000 ? 2 : 0)}Cr` : `₹${Math.round(n / 100000)}L`;
        ...
        <div className="p-4 md:p-6 space-y-4 md:space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between text-xs text-slate-400">
              <span>{shortInr(sliderMin)}</span>
              <span>{shortInr(sliderMax)}</span>
            </div>
