...
import { formatCurrency, normalizeAdditionalCharges } from "@/lib/formatters";
...
if (amountPaid > 0 && saleData) {
  ...
  const savedCharges = normalizeAdditionalCharges((sale as any).additional_charges);
...
const getPaymentBadgeColor = (status: string) => {
  ...
  const handleDownloadInvoice = async (sale: Sale) => {
    const toastOpts = { title: "Preparing invoice…", description: `Sale ${sale.sale_number}` };
    toast(toastOpts);
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) throw new Error("You must be signed in to download invoices.");

      // Fetch dealer settings, vehicle, customer, and EMI schedules in parallel.
      // Fall back to DB lookups for vehicle/customer so invoices work even when the
      // related record is missing from the in-memory list (e.g. archived/deleted).
      const [dealerRes, vehicleRes, customerRes, emiRes] = await Promise.all([
        supabase.from("settings")
          .select("dealer_name, dealer_address, dealer_phone, dealer_email, dealer_gst")
          .eq("user_id", user.id).maybeSingle(),
        (async () => {
          const cached = getVehicle(sale.vehicle_id);
          if (cached) return { data: cached, error: null };
          return supabase.from("vehicles")
            .select("brand, model, variant, registration_number, engine_number, chassis_number, manufacturing_year, color, fuel_type, transmission")
            .eq("id", sale.vehicle_id).maybeSingle();
        })(),
        (async () => {
          const cached = getCustomer(sale.customer_id);
          if (cached) return { data: cached, error: null };
          return supabase.from("customers")
            .select("full_name, phone, email, address")
            .eq("id", sale.customer_id).maybeSingle();
        })(),
        supabase.from("emi_schedules").select("*").eq("sale_id", sale.id).order("emi_number", { ascending: true }),
      ]);

      const vehicle = vehicleRes.data as any;
      const customer = customerRes.data as any;
      if (!vehicle) throw new Error("The vehicle linked to this sale could not be found.");
      if (!customer) throw new Error("The customer linked to this sale could not be found.");

      const dealer = {
        dealer_name: dealerRes.data?.dealer_name || "Your Dealership",
        dealer_address: dealerRes.data?.dealer_address || "",
        dealer_phone: dealerRes.data?.dealer_phone || "",
        dealer_email: dealerRes.data?.dealer_email || "",
        dealer_gst: dealerRes.data?.dealer_gst || "",
      };

      const schedules = emiRes.data || [];
      const firstEmi: any = schedules[0];
      const lastEmi: any = schedules[schedules.length - 1];
      const emiStartDate = firstEmi?.due_date;
      const emiEndDate = lastEmi?.due_date;
      const emiDueDay = emiStartDate ? new Date(emiStartDate).getDate() : undefined;
      ...
      generateSaleInvoicePDF(
        ...
        },
        {
          ...
          additional_charges: normalizeAdditionalCharges((sale as any).additional_charges),
          emi_tenure: schedules.length,
          ...
          emi_interest_rate:
            firstEmi && firstEmi.principal_component
              ? Math.round(((firstEmi.interest_component || 0) / firstEmi.principal_component) * 100)
              : 0,
        }
      );

      toast({ title: "Invoice downloaded", description: `Invoice_${sale.sale_number}.pdf` });
    } catch (err: any) {
      console.error("Invoice download failed", err);
      toast({
        title: "Invoice download failed",
        description: err?.message || "Something went wrong while generating the PDF. Please try again.",
        variant: "destructive",
      });
    }
  };
    ...
    >
      <FileText className="h-4 w-4" />
              ...
              <div className="space-y-4">
                ...
                {/* Additional Charges */}
                {normalizeAdditionalCharges((selectedSale as any).additional_charges).length > 0 && (
                  <div className="p-4 border rounded-lg">
                    <p className="text-sm font-medium mb-2">Additional Charges</p>
                    <div className="space-y-1">
                      {normalizeAdditionalCharges((selectedSale as any).additional_charges).map((c, i) => (
                      ...
                      <div className="flex justify-between pt-2 mt-2 border-t text-sm font-semibold">
                        ...
                        <span>
                          {formatCurrency(
                            normalizeAdditionalCharges((selectedSale as any).additional_charges)
