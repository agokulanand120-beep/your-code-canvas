...
const PublicVehiclePage = () => {
  ...
  const handleEnquirySubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      trackVehicleLead(getMetaVehicle(), { source: "website" });
    } catch (error: any) {
  ...
  const handleCall = () => {
    if (dealer?.dealer_phone) {
      trackPublicEvent({ eventType: "cta_call", dealerUserId: vehicle!.user_id, publicPageId: dealer?.public_page_id || "", vehicleId: vehicle!.id });
  ...
  const handleWhatsApp = () => {
    if (dealer?.whatsapp_number) {
      trackPublicEvent({ eventType: "cta_whatsapp", dealerUserId: vehicle!.user_id, publicPageId: dealer?.public_page_id || "", vehicleId: vehicle!.id });
    ...
    <div className={`min-h-screen ${pageBg}`}>
          ...
          {(dealer?.dealer_phone || dealer?.whatsapp_number) && (
            <div className="hidden lg:flex gap-2">
              ...
              {dealer?.whatsapp_number && (
                <a href={`https://wa.me/${dealer.whatsapp_number.replace(/\D/g, "")}?text=${getWhatsAppMessage()}`} target="_blank" rel="noopener noreferrer" onClick={handleWhatsApp}>
      ...
      <footer className="border-t border-gray-200 mt-16 pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 text-center">
          <p className={`text-sm ${textMuted}`}>© {new Date().getFullYear()} {dealer?.dealer_name}. Powered by UpcurvHub</p>
        </div>
      </footer>
    </div>
  );
