...
const PublicPageAnalytics = ({ userId: overrideUserId }: { userId?: string } = {}) => {
  const { user } = useAuth();
  ...
  const { data: settingsData } = useQuery({
    queryKey: ["catalogue-settings", effectiveUserId],
    queryFn: async () => {
  ...
  const { stats, trend, vehicleStats, hourStats, ctaStats, comparison } = useMemo(() => {
    ...
    vehicles.forEach(v => {
      vMap[v.id] = { vehicle_id: v.id, brand: v.brand, model: v.model, status: v.status as any, views: 0, uniqueVisitors: 0, enquiries: 0, ctaClicks: 0, clickRate: 0, conversion: 0 };
    });
