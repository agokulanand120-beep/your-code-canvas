...
import { extractDistrict, districtsFromAddresses } from "@/lib/location";
...
const AllVehicles = () => {
  ...
  const [vehicleType, setVehicleType] = useState(searchParams.get("body") || "all");
  const [fuelType, setFuelType] = useState(searchParams.get("fuel") || "all");
  const initialBudget = searchParams.get("budget");
  const [priceRange, setPriceRange] = useState<[number, number]>(() => {
    if (initialBudget) {
      const [lo, hi] = initialBudget.split("-").map(Number);
      if (!isNaN(lo) && !isNaN(hi)) return [lo, hi];
    }
    return [50000, 7500000];
  });
  ...
  // Debounced Meta Pixel Search event
  useEffect(() => {
  ...
  // City filters operate on DISTRICT only (see src/lib/location.ts)
  const getDealerCity = useCallback((userId: string) => {
    const dealer = dealers.find(d => d.user_id === userId);
    return extractDistrict(dealer?.dealer_address);
  }, [dealers]);

  const availableCities = useMemo(
    () => districtsFromAddresses(dealers.map(d => d.dealer_address)),
    [dealers]
  );
  ...
  return (
    <div className="min-h-screen bg-background pb-20 md:pb-0">
      ...
      <header className="sticky top-14 md:top-16 z-40 bg-background/95 backdrop-blur-lg border-b border-border">
