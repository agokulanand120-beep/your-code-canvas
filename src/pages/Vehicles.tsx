...
import { Plus, Pencil, Trash2, Search, Eye, ChevronLeft, ChevronRight, Upload, X, Image, FileText, Download, ExternalLink, Globe, Copy, Link, Printer, Store, Star } from "lucide-react";
...
import { vehicleSlug } from "@/lib/seoSlug";
...
const Vehicles = () => {
  ...
  const [primaryPendingIndex, setPrimaryPendingIndex] = useState<number | null>(null);
...
if (hasPurchasePayment) {
  ...
  const uploadImages = async (vehicleId: string, userId: string) => {
    ...
    try {
      // Use the vehicle's SEO slug in the stored filename so Google Images can
      // read the make/model/year straight from the image URL.
      const { data: vehicleRow } = await supabase
        .from("vehicles")
        .select("id, slug, brand, model, variant, manufacturing_year")
        .eq("id", vehicleId)
        .maybeSingle();
      const slugBase = vehicleRow ? vehicleSlug(vehicleRow as any) : vehicleId;
      const existingCount = vehicleImages[vehicleId]?.length || 0;

      for (const [index, file] of pendingImages.entries()) {
        const fileExt = (file.name.split('.').pop() || "jpg").toLowerCase().replace(/[^a-z0-9]/g, "");
        const fileName = `${vehicleId}/${slugBase}-${existingCount + index + 1}-${Date.now()}.${fileExt}`;
        ...
        const starred = primaryPendingIndex === index;
        const makePrimary = starred || (primaryPendingIndex === null && index === 0);

        const { data: inserted } = await supabase.from("vehicle_images").insert({
          vehicle_id: vehicleId,
          user_id: userId,
          image_url: publicUrl,
          display_order: existingCount + index,
          is_primary: existingCount === 0 && makePrimary,
        }).select("id").maybeSingle();

        // Dealer explicitly starred this new image — make it the profile photo.
        if (starred && existingCount > 0 && inserted?.id) {
          await supabase
            .from("vehicle_images")
...
const reorderExistingImages = async (imgs: VehicleImage[], fromIdx: number, toIdx: number) => {
  ...
  const setPrimaryExistingImage = async (img: VehicleImage) => {
    try {
      await supabase
        .from("vehicle_images")
        .update({ is_primary: false })
      ...
      toast({ title: "Could not set profile image", description: err.message, variant: "destructive" });
    }
  };

  const removePendingImage = (index: number) => {
    setPendingImages(pendingImages.filter((_, i) => i !== index));
    setPrimaryPendingIndex((prev) => {
...
{formData.purchase_status === "purchased" && !formData.vendor_id && (
  ...
  >
    ...
    {img.is_primary ? (
      <Badge className="absolute -top-2 -left-2 text-[10px] bg-amber-500 text-white">Profile</Badge>
    ) : (
      <Button
        type="button"
        size="icon"
                  ...
                  {pendingImages.length > 0 && (
                    <>
                      <p className="text-xs text-muted-foreground mt-2">
                        Tap the star to choose the profile (cover) image for this vehicle.
                      </p>
                      <div className="flex flex-wrap gap-2 mt-2">
                        {pendingImages.map((file, i) => {
                          const isCover =
                            primaryPendingIndex === i ||
                            (primaryPendingIndex === null && i === 0 && existingImages.length === 0);
                          return (
                            <div key={i} className="relative">
                              <img
                                src={URL.createObjectURL(file)}
                                alt=""
                                className={`h-16 w-20 object-cover rounded border-2 ${isCover ? "border-amber-500" : "border-transparent"}`}
                              />
                              {isCover && (
                                <Badge className="absolute bottom-0 left-0 text-[9px] px-1 py-0 rounded-none rounded-tr bg-amber-500 text-white">
                                  Profile
                                </Badge>
                              )}
                              <Button
                                ...
                                onClick={(e) => {
                                  e.preventDefault();
                                  e.stopPropagation();
                                  setPrimaryPendingIndex(i);
                                }}
                              >
                                <Star className={`h-3 w-3 ${isCover ? "fill-amber-500 text-amber-500" : ""}`} />
                              </Button>
...
>
      ...
      <Dialog open={detailDialogOpen} onOpenChange={setDetailDialogOpen}>
        <DialogContent className="max-w-3xl w-[calc(100vw-2rem)] max-h-[90vh] overflow-y-auto overflow-x-hidden p-0">
          {selectedVehicle && (() => {
            const images = vehicleImages[selectedVehicle.id] || [];
                ...
                <Tabs defaultValue="overview" className="w-full min-w-0">
                  <div className="px-4 pt-2 min-w-0">
                    <TabsList className="w-full grid grid-cols-4 h-9">
                      ...
                      <TabsTrigger value="docs" className="text-xs px-1">Docs ({docs.length})</TabsTrigger>
                  ...
                  <TabsContent value="more" className="px-4 pb-4 mt-0 pt-3 space-y-3 min-w-0 max-w-full overflow-hidden">
                    {selectedVehicle.is_public && dealerName && (
                      <div className="p-3 bg-muted/50 rounded-lg border border-border/50 space-y-2 min-w-0">
                        <div className="flex items-center gap-2">
                          <Globe className="h-4 w-4 text-green-600 shrink-0" />
                          <p className="text-xs font-medium">Public Page</p>
                        </div>
                        <code className="text-[11px] text-muted-foreground block w-full max-w-full break-all whitespace-normal line-clamp-2">{window.location.origin}{getVehicleCatalogueUrl(dealerName, selectedVehicle.code, selectedVehicle.id)}</code>
                        <div className="flex flex-wrap gap-2">
                    ...
                    <Button variant="outline" size="sm" className="gap-2 w-full max-w-full justify-start" onClick={async () => {
