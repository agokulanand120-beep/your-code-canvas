...
const DealerPublicPage = () => {
  ...
  // Some routes only give us dealerSlug (e.g. /catalogue/:dealerSlug). Once dealerInfo loads,
  // fall back to its stored public_page_id so analytics events attribute correctly.
  const [resolvedPageId, setResolvedPageId] = useState<string>(pageId || "");
  ...
  const handleInlineFormFocus = useCallback(() => {
    if (!formOpened && dealerInfo?.user_id && pageId) {
      setFormOpened(true);
      trackPublicEvent({ eventType: "form_opened", dealerUserId: dealerInfo.user_id, publicPageId: resolvedPageId });
    }
  ...
  useEffect(() => {
    return () => {
      if (formOpened && !submitted && dealerInfo?.user_id && pageId) {
        trackPublicEvent({ eventType: "form_abandoned", dealerUserId: dealerInfo.user_id, publicPageId: resolvedPageId });
      }
    };
  ...
  const fetchDealerData = async () => {
    try {
      ...
      const effectivePageId = pageId || settings.public_page_id || "";
      setResolvedPageId(effectivePageId);

      await trackPublicEvent({ eventType: "page_view", dealerUserId: settings.user_id, publicPageId: effectivePageId });
  ...
  const handleEnquiry = async () => {
    ...
    try {
      ...
      await trackPublicEvent({ eventType: "enquiry_submit", dealerUserId: dealerInfo.user_id, publicPageId: resolvedPageId });
    } catch (error: any) {
    ...
    <div className={`min-h-screen ${pageBg}`}>
          ...
          <div className="space-y-4">
            ...
            <Card className={`${cardBg} overflow-hidden`}>
              ...
              <CardContent className="p-4 space-y-2">
                {dealerInfo.dealer_phone && (
                  <a href={`tel:${dealerInfo.dealer_phone}`} onClick={() => trackPublicEvent({ eventType: "cta_call", dealerUserId: dealerInfo.user_id, publicPageId: resolvedPageId })}>
                ...
                {dealerInfo.whatsapp_number && (
                  <a href={`https://wa.me/${dealerInfo.whatsapp_number.replace(/\D/g, "")}`} target="_blank" rel="noopener noreferrer" onClick={() => trackPublicEvent({ eventType: "cta_whatsapp", dealerUserId: dealerInfo.user_id, publicPageId: resolvedPageId })}>
      ...
      <footer className={`border-t ${isPremium ? "border-gray-800" : "border-gray-200"} mt-12`}>
        <div className="w-full px-4 sm:px-6 py-6 text-center">
          <p className={`text-xs ${textMuted}`}>© {new Date().getFullYear()} {dealerInfo.dealer_name}. Powered by UpcurvHub</p>
        </div>
      </footer>
    </div>
  );
