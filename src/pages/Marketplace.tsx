...
import { mixedBrands } from "@/lib/brandLogos";
...
import { vehiclePath, slugify } from "@/lib/seoSlug";
...
const serviceCategories = [
  ...
  { icon: Banknote, label: "Car Loan", href: "/car-loan", color: "text-amber-600", bg: "bg-amber-50" },
  { icon: Shield, label: "Insurance", href: "/insurance", color: "text-rose-600", bg: "bg-rose-50" },
  { icon: Wrench, label: "Car Services", href: "/car-services", color: "text-cyan-600", bg: "bg-cyan-50" },
  { icon: Tag, label: "Accessories", href: "/accessories", color: "text-indigo-600", bg: "bg-indigo-50" },
];

// Home page brand rail: cars and bikes interleaved.
const popularBrands = mixedBrands;

// Hero strip shows only the first five logo-backed brands.
const heroBrands = popularBrands.slice(0, 5);
...
const happyStories = [
  { name: "Rajesh K.", city: "Coimbatore, Tamil Nadu", review: "Exchanged my car seamlessly, amazing experience! The team was very professional and guided me through the entire process.", avatar: "R", rating: 5 },
  { name: "Priya S.", city: "Chennai, Tamil Nadu", review: "Smooth buying process, got my dream car within a week! Highly recommend UpcurvHub to everyone.", avatar: "P", rating: 5 },
  { name: "Suresh M.", city: "Madurai, Tamil Nadu", review: "Best price for my old car, instant payment received. No hidden charges at all. Great platform!", avatar: "S", rating: 4 },
];
...
const Marketplace = () => {
  ...
  // Recently LISTED marketplace vehicles — ordered by when they went live on the
  // marketplace (marketplace_listed_at), not by when the vehicle was added to stock.
  const { data: latestVehicles = [] } = useQuery({
    queryKey: ["marketplace-latest-listed-vehicles"],
    queryFn: async () => {
      // Only show vehicles that are BOTH catalogue-visible (is_public) AND marketplace-approved.
      const { data, error } = await supabase
        .from("vehicles")
        .select("id, brand, model, variant, manufacturing_year, selling_price, odometer_reading, fuel_type, code, marketplace_listed_at")
        .eq("is_public", true)
        .eq("status", "in_stock")
  ...
  // Remember the last brand the user drilled into so coming back keeps the
  // selection visible, along with the chosen city.
  const [selectedBrand, setSelectedBrand] = useState<string | null>(
    () => sessionStorage.getItem("marketplace_brand")
  );

  const handleBrandClick = (brandName: string) => {
    const next = selectedBrand === brandName ? null : brandName;
    setSelectedBrand(next);
    if (next) sessionStorage.setItem("marketplace_brand", next);
    else sessionStorage.removeItem("marketplace_brand");
    if (!next) return;
    // Dedicated, indexable brand hub instead of a filtered search URL.
    navigate(`/marketplace/brand/${slugify(brandName)}`);
  };
  ...
  return (
    <div className="min-h-screen bg-background pb-20 md:pb-0 overflow-x-hidden">
      ...
      {/* ───── BRAND LOGOS STRIP (tap to filter by brand + city) ───── */}
      <section className="container mx-auto px-3 md:px-4 pt-4 md:pt-6">
        <div className="flex items-center justify-between mb-3 md:mb-4">
          <div className="flex items-center gap-2 min-w-0">
            <h2 className="text-base md:text-xl font-bold text-foreground">Explore by Brand</h2>
            {selectedBrand && (
              <button
                onClick={() => handleBrandClick(selectedBrand)}
                className="text-[10px] md:text-xs font-semibold text-blue-700 bg-blue-50 border border-blue-200 rounded-full px-2.5 py-0.5 truncate"
              >
                {selectedBrand}
                {cityFilter && cityFilter !== "all" ? ` · ${cityFilter}` : ""} ✕
              </button>
            )}
          </div>
          <Link to="/marketplace/brands" className="text-xs md:text-sm text-blue-600 hover:text-blue-700 font-semibold inline-flex items-center gap-1 shrink-0">
          ...
          {popularBrands.map((brand) => {
            ...
            return (
              <button
                ...
                aria-label={`Browse ${brand.name} vehicles${cityFilter && cityFilter !== "all" ? ` in ${cityFilter}` : ""}`}
                className="flex flex-col items-center gap-2 min-w-[76px] md:min-w-[96px] group shrink-0"
              >
                <div
                  className={`h-16 w-16 md:h-20 md:w-20 rounded-2xl flex items-center justify-center transition-all duration-200 overflow-hidden border-2 ${
                    isSelected
                      ? "bg-blue-50 border-blue-600 shadow-lg ring-2 ring-blue-200 -translate-y-0.5"
                      : "bg-card border-border group-hover:border-blue-500 group-hover:shadow-lg group-hover:-translate-y-0.5"
                  }`}
                >
                  {brand.logo ? (
                    <img
                      ...
                      onError={(e) => {
                        const el = e.target as HTMLImageElement;
                        el.style.display = "none";
                        el.parentElement!.innerHTML = `<span class="text-sm font-bold text-muted-foreground">${brand.name.slice(0, 2).toUpperCase()}</span>`;
                      }}
                    />
                  ) : (
                    <span className={`text-sm md:text-base font-extrabold tracking-tight ${isSelected ? "text-blue-700" : "text-muted-foreground"}`}>
                      {brand.name.replace(/[^A-Za-z]/g, "").slice(0, 3).toUpperCase()}
                    </span>
                  )}
                </div>
                <span className={`text-[10px] md:text-xs font-semibold text-center leading-tight ${isSelected ? "text-blue-700" : "text-foreground group-hover:text-blue-600"}`}>
                  {brand.name}
                </span>
              </button>
            );
      ...
      {/* ───── RECENTLY LISTED ON MARKETPLACE (LIVE, LATEST 4) ───── */}
      {latestVehicles.length > 0 && (
        <section className="container mx-auto px-3 md:px-4 pt-6 md:pt-8">
          <div className="flex items-center justify-between mb-4 md:mb-6">
            <div className="flex items-center gap-3">
              <h2 className="text-lg md:text-2xl font-bold text-foreground">Recently Listed</h2>
      ...
      {/* ───── PRODUCTS STORE BANNER ───── */}
      <section className="container mx-auto px-3 md:px-4 py-8 md:py-12">
        <Link
          to="/shop"
          className="group block rounded-3xl overflow-hidden border border-border bg-gradient-to-br from-blue-600 via-indigo-600 to-blue-700 text-primary-foreground shadow-lg hover:shadow-xl transition-shadow"
        >
          <div className="p-6 md:p-10 flex flex-col md:flex-row md:items-center gap-6">
            <div className="flex-1 min-w-0">
              <span className="inline-flex items-center gap-1.5 text-[10px] md:text-xs font-semibold uppercase tracking-wider bg-white/15 rounded-full px-3 py-1">
                <Tag className="h-3 w-3" /> Products Store
              </span>
              <h2 className="mt-3 text-xl md:text-3xl font-bold leading-tight">
                Car &amp; bike essentials, handpicked and priced right
              </h2>
              <p className="mt-2 text-sm md:text-base text-primary-foreground/85 max-w-2xl">
                Dashcams, car care, helmets, tyre inflators, seat covers and more — curated by our team,
                with trusted brands and secure checkout.
              </p>
              <div className="mt-4 flex flex-wrap gap-3 md:gap-5">
                {[
                  { icon: ShieldCheck, text: "Genuine brands only" },
                  { icon: Star, text: "Rated by real buyers" },
                  { icon: Wrench, text: "Fitment help included" },
                ].map((f, i) => (
                  <span key={i} className="inline-flex items-center gap-1.5 text-[11px] md:text-sm font-medium text-primary-foreground/90">
                    <f.icon className="h-3.5 w-3.5 md:h-4 md:w-4" /> {f.text}
                  </span>
                ))}
              </div>
            </div>
            <div className="shrink-0">
              <span className="inline-flex items-center gap-2 rounded-xl bg-white text-blue-700 font-semibold px-5 py-3 text-sm md:text-base group-hover:gap-3 transition-all">
      ...
      <section className="container mx-auto px-3 md:px-4 py-8 md:py-12">
        <div className="flex items-center gap-3 mb-6">
          <h2 className="text-lg md:text-2xl font-bold text-foreground">
            <span className="text-blue-600">10+</span> Happy Drive Stories
          </h2>
          <div className="flex-1 h-px bg-gradient-to-r from-blue-200 to-transparent" />
        </div>
        ...
        <div className="bg-card rounded-2xl md:rounded-3xl border border-border shadow-sm overflow-hidden">
          <div className="grid grid-cols-1 md:grid-cols-2">
            ...
            <div className="p-6 md:p-10 flex flex-col justify-center">
              ...
              <h2 className="text-2xl md:text-3xl font-bold text-foreground mb-3">Grow Your Business with UpcurvHub</h2>
            ...
            <div className="bg-muted/30 p-6 md:p-10 flex flex-col justify-center border-t md:border-t-0 md:border-l border-border">
              ...
              <div className="space-y-4">
                {[
                  { name: "AutoMax Motors", city: "Coimbatore, Tamil Nadu", rating: 5, text: "UpcurvHub doubled our online leads in just 2 months!" },
                  { name: "Prime Cars", city: "Chennai, Tamil Nadu", rating: 5, text: "Best platform for used car dealers. Highly recommend!" },
                  { name: "Royal Wheels", city: "Salem, Tamil Nadu", rating: 4, text: "Easy to manage inventory and connect with genuine buyers." },
                ].map((review, i) => (
