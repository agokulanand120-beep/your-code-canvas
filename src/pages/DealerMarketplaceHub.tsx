    ...
    queryFn: async () => {
      ...
      const [{ data: events }, { count: publicVehiclesCount }] = await Promise.all([
        supabase
          ...
          .select("event_type, vehicle_id, created_at, session_id")
      ...
      const sessions: Record<string, { first: number; last: number; types: Set<string> }> = {};
      allEvents.forEach((e: any) => {
        const sid = e.session_id || "anon";
        const t = new Date(e.created_at).getTime();
        if (!sessions[sid]) sessions[sid] = { first: t, last: t, types: new Set() };
        sessions[sid].first = Math.min(sessions[sid].first, t);
        sessions[sid].last = Math.max(sessions[sid].last, t);
        sessions[sid].types.add(e.event_type);
      });
      const sessionList = Object.values(sessions);
      const visitors = sessionList.length;
      const durations = sessionList.map(s => Math.min((s.last - s.first) / 1000, 1800));
      const avgTimeOnPage = durations.length ? durations.reduce((a, b) => a + b, 0) / durations.length : 0;
      const engagedSessions = sessionList.filter(s =>
        s.types.has("engaged_30s") || s.types.has("engaged_60s") || s.types.has("engaged_120s")
      ).length;
      const deepScrollSessions = sessionList.filter(s => s.types.has("scroll_75") || s.types.has("scroll_100")).length;
      const highIntent = sessionList.filter(s =>
        s.types.has("enquiry_submit") || s.types.has("cta_call") || s.types.has("cta_whatsapp")
      ).length;
      const mediumIntent = sessionList.filter(s =>
        !(s.types.has("enquiry_submit") || s.types.has("cta_call") || s.types.has("cta_whatsapp")) &&
        (s.types.has("form_opened") || s.types.has("scroll_75") || s.types.has("scroll_100") || s.types.has("engaged_30s"))
      ).length;
      const lowIntent = Math.max(visitors - highIntent - mediumIntent, 0);
      const bouncedSessions = sessionList.filter(s => s.last - s.first < 10000 && s.types.size <= 1).length;
      ...
      return {
        ...
        engagement: {
          visitors,
          avgTimeOnPage,
          engagedRate: visitors ? (engagedSessions / visitors) * 100 : 0,
          deepScrollRate: visitors ? (deepScrollSessions / visitors) * 100 : 0,
          bounceRate: visitors ? (bouncedSessions / visitors) * 100 : 0,
          highIntent,
          mediumIntent,
          lowIntent,
        },
...
const DealerMarketplaceHub = () => {
  ...
  const engagement = analyticsData?.engagement || {
    visitors: 0, avgTimeOnPage: 0, engagedRate: 0, deepScrollRate: 0,
    bounceRate: 0, highIntent: 0, mediumIntent: 0, lowIntent: 0,
  };
        ...
        <div className="space-y-6 animate-fade-in">
          ...
          <Card className="border shadow-sm">
            <CardHeader>
              ...
              <CardDescription>How long visitors stay on your marketplace pages and how strong their buying intent is</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  { label: "Unique Visitors", value: engagement.visitors },
                  { label: "Avg. Time on Page", value: formatDuration(engagement.avgTimeOnPage) },
                  { label: "Engaged (30s+)", value: `${engagement.engagedRate.toFixed(0)}%` },
                  { label: "Read 75%+ of Page", value: `${engagement.deepScrollRate.toFixed(0)}%` },
                ].map((m, i) => (
              ...
              <div className="space-y-3">
                {[
                  { label: "High intent", hint: "called, WhatsApp'd or enquired", value: engagement.highIntent, cls: "bg-emerald-500" },
                  { label: "Medium intent", hint: "opened form, scrolled deep or stayed 30s+", value: engagement.mediumIntent, cls: "bg-amber-500" },
                  { label: "Low intent", hint: "quick look only", value: engagement.lowIntent, cls: "bg-slate-400" },
                ].map((row, i) => {
                  const pct = engagement.visitors ? (row.value / engagement.visitors) * 100 : 0;
                  return (
                    <div key={i} className="space-y-1">
                      <div className="flex items-center justify-between text-sm">
                        <span className="font-medium">
                          {row.label} <span className="text-muted-foreground font-normal">— {row.hint}</span>
                        </span>
                        <span className="tabular-nums text-muted-foreground">{row.value} ({pct.toFixed(0)}%)</span>
                      </div>
              ...
              <p className="text-xs text-muted-foreground">
                Bounce rate (left within 10 seconds): <span className="font-medium text-foreground">{engagement.bounceRate.toFixed(0)}%</span>
              </p>
            </CardContent>
          </Card>
