...
async function fetchVehiclePageData(userId: string): Promise<VehiclePageData> {
  const [vehiclesRes, vendorsRes, imagesRes, docsRes, settingsRes] = await Promise.all([
    ...
    supabase.from("vehicle_images").select("id, vehicle_id, image_url, is_primary, display_order, user_id, created_at").eq("user_id", userId),
    supabase.from("documents").select("id, reference_id, document_name, document_type, document_url, status, expiry_date, notes, reference_type, updated_at, user_id, created_at, folder_path").eq("reference_type", "vehicle").eq("user_id", userId),
