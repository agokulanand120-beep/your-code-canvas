...
export const carBrands: VehicleBrand[] = [
  {
    ...
    models: [
      ...
      { name: "S-Cross", variants: [{ name: "Sigma" }, { name: "Delta" }, { name: "Zeta" }, { name: "Alpha" }, { name: "DDiS 200 Sigma" }, { name: "DDiS 200 Delta" }, { name: "DDiS 200 Zeta" }, { name: "DDiS 200 Alpha" }] },
    ],
  },
  {
    name: "Hyundai",
...
// Helper functions
import { getExtraBrands } from "./vehicleDataExtra";
...
// Admin-managed additions loaded from the database at runtime.
let catalogOverlay: CatalogOverlayEntry[] = [];

export const setCatalogOverlay = (entries: CatalogOverlayEntry[]) => {
  catalogOverlay = entries || [];
};

const overlayToBrands = (vehicleType: string): VehicleBrand[] => {
  const map = new Map<string, Map<string, Set<string>>>();
  catalogOverlay
    .filter((e) => (e.vehicle_type || "car") === vehicleType && e.brand)
    .forEach((e) => {
      if (!map.has(e.brand)) map.set(e.brand, new Map());
      const models = map.get(e.brand)!;
      if (e.model) {
        if (!models.has(e.model)) models.set(e.model, new Set());
        if (e.variant) models.get(e.model)!.add(e.variant);
      }
    });
  return Array.from(map.entries()).map(([name, models]) => ({
    name,
    models: Array.from(models.entries()).map(([mName, variants]) => ({
      name: mName,
      variants: Array.from(variants).map((v) => ({ name: v })),
    })),
  }));
};

const mergeBrandLists = (...lists: VehicleBrand[][]): VehicleBrand[] => {
  const merged: VehicleBrand[] = [];
  const brandIdx = new Map<string, VehicleBrand>();

  lists.flat().forEach((brand) => {
    let existing = brandIdx.get(brand.name);
    if (!existing) {
      existing = { name: brand.name, models: [] };
      brandIdx.set(brand.name, existing);
      merged.push(existing);
    }
    brand.models.forEach((model) => {
      let m = existing!.models.find((x) => x.name === model.name);
      if (!m) {
        m = { name: model.name, variants: [] };
        existing!.models.push(m);
      }
      model.variants.forEach((variant) => {
        if (!m!.variants.some((x) => x.name === variant.name)) {
  ...
  merged.forEach((b) => {
    b.models.sort((a, z) => a.name.localeCompare(z.name));
  });
  return merged.sort((a, z) => a.name.localeCompare(z.name));
};

const baseBrandsForType = (vehicleType: string): VehicleBrand[] => {
  switch (vehicleType) {
    case 'car':
      return carBrands;
    case 'bike':
...
export const getBrandsForType = (vehicleType: string): VehicleBrand[] => {
  ...
  return mergeBrandLists(
    baseBrandsForType(type),
    getExtraBrands(type),
    overlayToBrands(type)
  );
};
