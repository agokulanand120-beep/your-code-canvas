...
import { formatIndianNumber, normalizeAdditionalCharges } from './formatters';
...
if (customer.address) {
  ...
  const charges = normalizeAdditionalCharges(sale.additional_charges);
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      ...charges.map((c) => [c.name || 'Additional Charge', formatRupees(c.amount)]),
