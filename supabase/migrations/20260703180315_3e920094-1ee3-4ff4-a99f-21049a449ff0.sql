...
-- Allow public reads of vehicles when either catalogue toggle (is_public) is on
-- OR the vehicle is listed on the marketplace (approved/pending). Previously
-- required is_public which blocked marketplace-only listings.
DROP POLICY IF EXISTS "Public can read public vehicles" ON public.vehicles;
CREATE POLICY "Public can read public vehicles"
  ON public.vehicles FOR SELECT
  TO public
  USING (
    is_public = true
    OR marketplace_status IN ('approved','pending','listed')
  );

-- Dashboard summary: add pending_vendor_payments (unpaid balances on purchases)
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'pending_vendor_payments', (SELECT COALESCE(SUM(balance_amount),0) FROM vehicle_purchases WHERE user_id = uid),
